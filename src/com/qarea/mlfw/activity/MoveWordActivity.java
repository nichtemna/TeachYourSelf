...
public class MoveWordActivity extends BaseActivity implements OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_move_word);
        setTitle(getResources().getString(R.string.app_name));

        Intent intent = getIntent();
        currentDate = intent.getLongExtra("currentDate", 0);
        textForButtonLeft = getResources().getString(R.string.move);
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            // send words to change date
            case R.id.button_left:
                if (getCheckedItemsCount(checkbox_list) == 0) {
                    Toast.makeText(MoveWordActivity.this,
                    ...
                    for (int i = 0; i < checked.size(); i++) {
                        ...
                        if (checked.valueAt(i)) {
                            words_move.add((Word) adapter.getItem(pos));
                        }
                    }
                    Intent getDateIntent = new Intent(MoveWordActivity.this,
                                    MoveWordCalendarActivity.class);
                    getDateIntent.putExtra(BaseActivity.MOVE_ARRAY_WORDS, words_move);
                    getDateIntent.putExtra(BaseActivity.MOVE_TIME, currentDate);
                    getDateIntent.putExtra(BaseActivity.REPEATE_WORD,
                                    BaseActivity.REPEATE_WORD_MOVE);
                    startActivityForResult(getDateIntent, TAG_FOR_CALENDAR);
                    finish();
                }
                break;
            case R.id.button_rigth:
                // start calendar for date
                finish();
                break;
        }
    }
